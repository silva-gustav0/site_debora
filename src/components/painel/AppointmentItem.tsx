...
export default function AppointmentItem({ appt, showDate = false }: { appt: AppointmentWithRefs; showDate?: boolean }) {
  ...
  return (
    <Link
      ...
      className={`group flex items-center gap-3 rounded-xl bg-white border border-[#F0E8DB] px-3 py-2.5 transition-all hover:border-[#E6D8BC] hover:shadow-[0_8px_20px_-14px_rgba(43,34,27,.4)] ${faded ? "opacity-60" : ""}`}
      style={{ borderLeft: `3px solid ${STATUS_BORDER[appt.status]}` }}
    >
      <div className="w-14 shrink-0 p-num">
        {showDate && <span className="block text-[11px] text-[#857566]">{fmtDate(day, { year: undefined })}</span>}
        <span className="text-[15px] font-bold text-[#2B221B]">{fmtTime(appt.starts_at)}</span>
      </div>
      ...
      <ChevronRight size={15} className="text-[#CFC3AE] group-hover:text-[#6B4A10] transition-colors" />
    </Link>
  );
}
