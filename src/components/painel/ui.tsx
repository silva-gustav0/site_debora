  ...
  return (
    <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-7 p-rise">
      <div>
        ...
        <h1 className="p-display text-[2.4rem] sm:text-5xl font-light text-[#2B221B] leading-[1.05]">{title}</h1>
        {subtitle && <p className="text-sm text-[#857566] mt-2">{subtitle}</p>}
      </div>
...
export function Card({
  ...
  return (
    <section className={`p-card ${gold ? "p-card-gold" : ""} ${className}`}>
      {title && (
        <div className="flex items-center justify-between gap-3 px-5 pt-4 pb-3 border-b border-[#F5EEE3]">
          <div>
            {eyebrow && <p className="p-eyebrow text-[9.5px] mb-0.5">{eyebrow}</p>}
            <h2 className="p-display text-[1.45rem] leading-tight font-normal text-[#2B221B]">{title}</h2>
          </div>
          {action}
        </div>
      )}
...
export function StatTile({
  ...
  const body = (
    <div
      ...
      style={dark ? { background: "linear-gradient(140deg,#2B221B 0%,#48392B 100%)", borderColor: "#382C22" } : undefined}
    >
      <div className="flex items-start justify-between gap-2">
        <p className={`text-[10.5px] uppercase tracking-[0.14em] font-bold ${dark ? "text-[#E8C882]" : "text-[#9A8B78]"}`}>{label}</p>
        {Icon && (
          <span
            className="w-8 h-8 rounded-full flex items-center justify-center"
            style={dark ? { background: "rgba(232,200,130,.14)", color: "#E8C882" } : { background: "#FDF7EC", color: "#B3842E" }}
          >
            <Icon size={15} />
          </span>
        )}
      ...
      <p
        className={`p-display p-num text-[2.1rem] leading-none mt-2 ${dark ? "text-white" : tone === "warn" ? "text-[#9A5B00]" : "text-[#2B221B]"}`}
      >
        {value}
      </p>
      <div className={`flex items-center gap-2 text-xs mt-2 ${dark ? "text-[#E8DCC9]/70" : "text-[#857566]"}`}>
...
export const STATUS_BORDER: Record<AppointmentStatus, string> = {
  solicitado: "#D4A73C", confirmado: "#6A7BC9", concluido: "#3F9A5E", cancelado: "#B8AFA2", faltou: "#C0504D",
};
...
const AVATAR_BG = ["#9A6F1E", "#A87B25", "#8E4BA0", "#5B6FC9", "#3F9A5E", "#B85C38", "#6B5A4B", "#2F7A8A"];
...
export function EmptyState({ icon: Icon, children }: { icon?: LucideIcon; children: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-8 px-4 text-sm text-[#857566]">
      {Icon && (
        <span className="w-11 h-11 rounded-full flex items-center justify-center mb-3" style={{ background: "#FDF7EC", color: "#C9973A" }}>
          <Icon size={18} />
        </span>
      )}
      {children}
...
export function Progress({ value, max, color = "#9A6F1E" }: { value: number; max: number; color?: string }) {
  const pct = max > 0 ? Math.min(100, Math.round((value / max) * 100)) : 0;
  return (
    <div className="h-1.5 rounded-full bg-[#F4EDE1] overflow-hidden" role="progressbar" aria-valuenow={value} aria-valuemin={0} aria-valuemax={max}>
